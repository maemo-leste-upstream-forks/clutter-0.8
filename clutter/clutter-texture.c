...
clutter_texture_init (ClutterTexture *self)
{
  ...
  priv->filter_quality    = CLUTTER_TEXTURE_QUALITY_LOW;
  priv->repeat_x          = FALSE;
  priv->repeat_y          = FALSE;
  ...
  if (has_alpha)
    {
      if (bpp == 4)
        ...
        {
          source_format = COGL_PIXEL_FORMAT_RGBA_4444;
        }
      else if (bpp == 1)
        {
...
clutter_texture_set_from_file (ClutterTexture *texture,
                               ...
                               GError        **error)
{
  return clutter_texture_set_from_file_f(
                texture, filename,
                CLUTTER_DEFAULT_PIXEL_FORMAT,
                error);
}
 ...
 *
 * Sets the #ClutterTexture image data from an image file. In case of
 * failure, %FALSE is returned and @error is set. A pixel format can
 * be supplied that determines how the texture is to be stored in OpenGL.
 *
 * Return value: %TRUE if the image was successfully loaded and set
 *
 * Since: 0.8
 */
gboolean
clutter_texture_set_from_file_f (ClutterTexture *texture,
			       const gchar    *filename,
                               CoglPixelFormat internal_format,
			       GError        **error)
{
 ...
 **/
ClutterActor*
clutter_texture_new_from_file_f (const gchar *filename,
                                 CoglPixelFormat internal_format,
			         GError     **error)
{
  ClutterActor *texture = clutter_texture_new ();

  if (!clutter_texture_set_from_file_f (CLUTTER_TEXTURE (texture),
				      filename, internal_format, error))
    {
